...
MIMIC_BIN_FOLDER=$UPLOAD_FOLDER/zats-mimic/release/$VERSION/bin
mkdir -p $MIMIC_BIN_FOLDER

cp zats/target/zats-$VERSION-javadoc.jar $UPLOAD_FOLDER/zats/release/$VERSION/zats-javadoc-$VERSION.zip

cp zats/target/zats-mimic-$VERSION.zip $MIMIC_BIN_FOLDER

COMMON_MVN_FOLDER=$UPLOAD_FOLDER/zats-common/release/maven
mkdir -p $COMMON_MVN_FOLDER

cp zats-common/target/zats-common-$VERSION-bundle.jar $COMMON_MVN_FOLDER

MIMIC_MVN_FOLDER=$UPLOAD_FOLDER/zats-mimic/release/maven
mkdir -p $MIMIC_MVN_FOLDER

cp zats-mimic/target/zats-mimic-$VERSION-bundle.jar $MIMIC_MVN_FOLDER

MIMIC_EXT_MVN_FOLDER=$UPLOAD_FOLDER/zats-mimic-ext/release/maven
