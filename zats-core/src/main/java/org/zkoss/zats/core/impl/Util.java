...
public class Util
{
	 ...
	 * This method doesn't recognize beta or release candidate version.
	...
	public static BigInteger parseVersion(final String version)
	{
		...
		Matcher m = Pattern.compile("\\s*(\\d+)\\.(\\d+)\\.(\\d+)\\.?(\\d*)\\s*").matcher(version);
		if(!m.matches())
			return null;
		byte[] raw = new byte[4];
		for(int i = 0; i < m.groupCount(); ++i)
			raw[i] = m.group(i + 1).length() > 0 ? Byte.parseByte(m.group(i + 1)) : 0;
